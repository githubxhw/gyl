        ...
        <form id="form" method="post">
            <section class="content">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">填写期初单信息进行添加</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#save_button").click(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/qc/saveZhubAndZib",
                type: "post",
                data: params,
                dataType: "json",
                async: false,
                success: function (data) {
                    ...
                    if(resFlag == "1"){ //成功
                        alert("添加成功!");
                        window.location.href = "${pageContext.request.contextPath}/qc/getDdhToAddUI";
                    }else { //失败
                        alert("添加失败:"+msgList);
                    }
                },
        ...
        $("#toquery_button").click(function () {
            window.location.href = "${pageContext.request.contextPath}/pages/business/qc-query.jsp";
        });
    });
    ...
    function add_zib(x) {
        ...
        if (childCount == 0) {
            ...
            var text = $("#chbm-" + pos).val();                 /*????????????????注意可能需要修改*/
        ...
        if (flag == "1") {
            ...
            var str = '<tr id="' + nowCount + '">' +
                ...
                '                                </button>' +
                '</td>' +
                '<td>                            <input id="hh-' + nowCount + '" type="text" readonly="readonly"  class="form-control" name="qc_zibs[' + nowCount + '].hh"\n' +
                '                                   placeholder="行号" value="' + nowCount + '"></td>\n' +
                '<td>                            <input id="chbm-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].chbm"\n' +
                '                                   placeholder="存货编码(不为空)"></td>\n' +
                '<td>                            <input id="gg-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].gg"\n' +
                '                                   placeholder="规格" value=""></td>\n' +
                '<td>                            <input id="xh-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].xh"\n' +
                '                                   placeholder="型号" value=""></td>\n' +
                '<td>                            <input id="dw-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].dw"\n' +
                '                                   placeholder="单位" value=""></td>\n' +
                '<td>                            <input id="pc-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].pc"\n' +
                '                                   placeholder="批次" value=""></td >\n' +
                '<td>                            <input id="count-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].count"\n' +
                '                                   placeholder="数量" value=""></td >\n' +
                '<td>                            <input id="dj-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].dj"\n' +
                '                                   placeholder="单价" value=""></td>\n' +
                '<td>                            <input id="je-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].je"\n' +
                '                                   placeholder="金额" value=""></td>\n' +
                '<td>                            <input id="rkrq-' + nowCount + '" type="text" class="form-control" name="qc_zibs[' + nowCount + '].rkrq"\n' +
                '                                   placeholder="入库日期" value=""></td>\n' +
                '</tr>';
