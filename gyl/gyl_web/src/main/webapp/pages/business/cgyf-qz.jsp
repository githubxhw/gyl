            ...
            <div class="row data-type">
                ...
                <div class="col-md-4 data">
                    <div class="input-group date">
                        ...
                        <input id="sp_qzsj" type="text" class="form-control pull-right dateTimePicker" value="">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#query").click(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/yf/findBySxBzIsOne",
                type: "post",
                data: params,
                dataType: "json",
                async: false,
                success: function (data) {
                    ...
                    for (var i = 0; i < xsys_zhubs.length; i++) {
                        ...
                        html += '<tr>\n' +
                            '<td><input id="checkbox-pos-' + i + '" class="ids my-checkbox" name="zhub_ids" type="checkbox" value="' + xsys_zhubs[i].cgyfd_zhu_id+ '"></td>\n' +
                            '<td><button i="' + i + '" djh="' + xsys_zhubs[i].djh + '" checked="" ondblclick="find_zib(this)" zhubId="' + xsys_zhubs[i].cgyfd_zhu_id + '" style="width: 100%;height: 99%">' + xsys_zhubs[i].djh + '</button></td>\n' +
                            ...
                            '<td>' + FormatTime(xsys_zhubs[i].qzsj) + '</td>\n' +
                            '</tr>';
                    }
...
// 日期过滤
    ...
    function find_zib(x) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //给子表form的zhub_id赋值
                $("#zib_form").attr("zhub_id", yf_zhub_id);
                /*单据号：<input id="djh"...*/
                $("#djh").attr("value", djh);
                //添加内容
                ys_zibs = data;
    ...
    function sp_adopt(x) {
        ...
        if(djh.length <= 0 || djh == null || djh == undefined){
            ...
            if(confirm(msg) == true) {
                var qzr = $("#sp_qzr").val();
                var qzsj = $("#sp_qzsj").val();
                if(qzr.length <= 0 || qzr == null || qzr == undefined){
                    alert("请填写签字人!")
                }else {
                    if(qzsj.length <= 0 || qzsj == null || qzsj == undefined){
                        alert("请填写签字日期!")
                    }else {
                        $.ajax({
                            url: "${pageContext.request.contextPath}/yf/qz",
                            type: "GET",
                            data: {"djh":djh,"qzr":qzr,"qzsj":qzsj},
                            dataType: "json",
                            // async: false,
                            success: function (data) {
                                var pageInfo = data;
