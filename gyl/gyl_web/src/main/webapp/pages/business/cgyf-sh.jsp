...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <div class="pull-left">
            <div class="form-group form-inline">
                <div class="btn-group">
                    <button id="sp_forbidden_btn" onclick="sp(0)" type="button" class="btn bg-red">审批禁止</button>
                </div>
            </div>
        </div>
        <div class="pull-right">
            <div class="form-group form-inline">
                <div class="btn-group">
                    <button id="sp_adopt_btn" onclick="sp(1)" type="button" class="btn bg-green">审批通过</button>
                </div>
            </div>
        </div>
        <%--工具栏/--%>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#query").click(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/yf/findByNoSp",
                type: "post",
                data: params,
                dataType: "json",
                async: false,
                success: function (data) {
                    ...
                    for (var i = 0; i < cgyf_zhubs.length; i++) {
                        ...
                        html += '<tr>\n' +
                            '<td><input id="checkbox-pos-' + i + '" class="ids my-checkbox" name="zhub_ids" type="checkbox" value="' + cgyf_zhubs[i].xsysdzhubid+ '"></td>\n' +
...
// 日期过滤
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < ys_zibs.length; i++) {
                    html += '<tr>\n' +
                        '<td><input name="zib_ids" type="checkbox" value="' + ys_zibs[i].cgyfd_zib_id + '"></td>\n' +
                        ...
                        '<td>' + NoNull(ys_zibs[i].ljyfsl) + '</td>\n' +
                        '<td>' + statusbe(ys_zibs[i].status) + '</td>\n' +
                        '</tr>';
                }
                $("#zib_tbody").html(html);
    ...
    //功能：审批功能 ： 单击子表下面的 审批禁止或通过按钮 触发，给已经选择的订单修改信息
    //在服务器 判断是否该订单已经审批 若是，则抛出异常，否则 修改订单中 sxbz = 0或 1;
    function sp(sxbz) {
        var djh = $("#djh").attr("value");
        if(djh.length <= 0 || djh == null || djh == undefined){
            ...
            if(confirm(msg) == true){
                var spr = $("#sp_spr").val();
                var spsj = $("#sp_sprq").val();
                if(spr.length <= 0 || spr == null || spr == undefined){
                    alert("请填写审核人!")
                }else {
                    if(spsj.length <= 0 || spsj == null || spsj == undefined){
                        alert("请填写审核日期!")
                    }else {
                        $.ajax({
                            url: "${pageContext.request.contextPath}/yf/sp",
                            type: "GET",
                            data: {"djh":djh,"spr":spr,"spsj":spsj,"sxbz":sxbz},
                            dataType: "json",
                            success: function (data) {
                                var pageInfo = data;
                                if (pageInfo.resFlag == "1") {//成功
                                    ...
                                    location.href = "${pageContext.request.contextPath}/pages/business/cgyf-sh.jsp";
                                } else {//失败
