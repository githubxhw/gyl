            ...
            <section class="content">
                <div class="panel panel-default">
                    ...
                    <div class="row data-type">
                        <div class="col-md-2 title">入库号</div>
                                            ...
                                            <div class="form-horizontal">
                                                ...
                                                <form id="zib_save_form" method="post">
                                                    ...
                                                    <section class="content"> <!--产品信息-->
                                                        ...
                                                        <div class="panel panel-default">
                                                            <div class="panel-heading">采购入库单子单</div>
                                                            <div class="row data-type">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#query").click(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/findAllAndSfshIsZero",
                type: "post",
                data: params,
                dataType: "json",
                ...
                success: function (data) {
                    ...
                    var cgrkd_zhubs = data;
                    var html = '';
                    if(data==null){
                        alert("该到货单号不存在或已审批!")
                    }
                    for (var i = 0; i < cgrkd_zhubs.length; i++) {
                        html += '<tr>\n' +
                            ...
                            '                                                                <input id="zhub_xg_cgrkd_zhub_id-' + i + '" type="hidden" name="cgrkd_zhub_id" value="">\n' +
                            '                                                                \n' +
                            '                                                                <div class="col-md-2 title">入库号</div>\n' +
                            '                                                                <div class="col-md-4 data">\n' +
                            ...
                            '                                    </div>\n' +
                            '                                    <div class="modal-footer">\n' +
                            '                                        <button i="' + i + '" onclick="updateCgrkd_zhubById(this)" type="button" class="btn btn-outline" data-dismiss="modal">修改</button>\n' +
                            '                                        <button type="button" class="btn btn-outline" data-dismiss="modal">取消</button>\n' +
                            '                                    </div>\n' +
                            '                                </div>\n' +
                            '                                <!-- /.modal-content -->\n' +
                            ...
                            '                        </div>' +
                            <%--模拟窗口/--%>
                            '<button zhubId="' + cgrkd_zhubs[i].cgrkd_zhub_id + '" data-toggle="modal" data-target="#myModal_2" type="button" class="btn bg-olive btn-xs"\n' +
                    ...
                    $("#zhub_count").html("共 " + cgrkd_zhubs.length + " 条记录");
                    $("#zhub_count").attr("count", cgrkd_zhubs.length);
                },
                error: function () {
                    alert("查询失败！")
                }
        ...
        /*4*/
        //功能：主表：点击主表下面的添加按钮触发，添加到模态框主表信息，先查询出djh
        $("#get_zhub_djh").click(function () {
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/getZhub_djh",
                type: "get",
                data: {},
                dataType: "text",
                async: false,
        ...
        $("#zhub_save").click(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/saveCgrkd_zhub",
                type: "post",
                data: params,
                dataType: "text",
                //async: false,
        ...
        $("#get_zib_hh").click(function () {
            ...
            if (zhub_id == -1) {
                ...
                $.ajax({
                    url: "${pageContext.request.contextPath}/cgrkd/getZib_hh",
                    type: "get",
                    data: {"cgrkd_zhub_id": zhub_id},
                    dataType: "text",
                    async: false,
                    success: function (data) {
                        var hh = data;
        ...
        $("#zib_save").click(function () {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/saveZib",
                type: "post",
                data: params,
                dataType: "text",
                //async: false,
    ...
    function zhub_xg_btn(x) {
        //alert("ok")
        var cgrkd_zhub_id = $(x).attr("zhubId");
        $.ajax({
            url: "${pageContext.request.contextPath}/cgrkd/findCgrkd_zhubById",
            type: "get",
            data: {"cgrkd_zhub_id": cgrkd_zhub_id},
            dataType: "json",
            async: false,
            success: function (data) {
                //alert("ok");
    ...
    function updateCgrkd_zhubById(x) {
        //获取点击了该修改按钮所在的行号i
        var i = $(x).attr("i");
        //一定要移除disabled属性 ???????
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/cgrkd/updateCgrkd_zhubById",
            type: "post",
            data: params,
            dataType: "text",
            async: false,
    ...
    function delete_zhub(x) {
        ...
        if (confirm(msg) == true) {
            var cgrkd_zhub_id = $(x).attr("zhubId");
            console.log(cgrkd_zhub_id)
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/deleteCgrkd_ZhubById",
                type: "GET",
                data: {"cgrkd_zhub_id": cgrkd_zhub_id},
                dataType: "text",
                async: false,
    ...
    function deleteZhubByIds(x) {
        ...
        if (confirm(msg) == true) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/deleteCgrkd_ZhubByIds",
                type: "GET",
                data: {"ids": ids},
                dataType: "text",
                async: false,
    ...
    function find_zib(x) {
        ...
        var djh = $(x).attr("djh");
        $.ajax({
            url: "${pageContext.request.contextPath}/cgrkd/findCgrkd_zibByZhubId",
            type: "GET",
            data: {"cgrkd_zhub_id": cgrkd_zhub_id},
            dataType: "json",
            async: false,
            success: function (data) {
                ...
                $("#zib_tbody").innerHTML = '';
                //给子表form的zhub_id赋值
                $("#zib_form").attr("zhub_id", cgrkd_zhub_id);
                /*单据号：<input id="djh"...*/
                $("#djh").attr("value", djh);
                //添加内容
                cgrkd_zibs = data;
                ...
                for (var i = 0; i < cgrkd_zibs.length; i++) {
                    html += '<tr>\n' +
                        ...
                        '                        <div class="col-md-2 title">累计到货数量</div>\n' +
                        '                        <div class="col-md-4 data">\n' +
                        '                            <input readonly="readonly" id="zib_xg_ljrksl-' + i + '"  type="text" class="form-control" name="ljrksl"\n' +
                        '                                   placeholder="累计到货数量" value="">\n' +
                        '                        </div>' +
                        '                         <div class="col-md-2 title">状态</div>\n' +
                        '                         <div class="col-md-4 data">\n' +
                        ...
                        '                                    </div>\n' +
                        '                                    <div class="modal-footer">\n' +
                        '                                        <button zibId="' + cgrkd_zibs[i].xsdd_zib_id + '" i="' + i + '" onclick="updateZibById(this)" type="button" class="btn btn-outline" data-dismiss="modal">修改</button>\n' +
                        '                                        <button type="button" class="btn btn-outline" data-dismiss="modal">取消</button>\n' +
                        '                                    </div>\n' +
                        '                                </div>\n' +
                        '                                <!-- /.modal-content -->\n' +
                ...
                $("#zib_tbody").html(html);
                $("#zib_count").attr("count", cgrkd_zibs.length);
                $("#zib_count").html("共 " + cgrkd_zibs.length + " 条记录");
            },
            error: function () {
                alert("查询失败！")
            }
    ...
    function zib_xg_btn(x) {
        ...
        if(zib_status == 1){
            ...
            //没有结束
            var cgrkd_zib_id = $(x).attr("zibId");
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/findCgrkd_zibById",
                type: "get",
                data: {"cgrkd_zib_id": cgrkd_zib_id},
                dataType: "json",
                async: false,
                success: function (data) {
                    var cgrkd_zib = data;
    ...
    function updateZibById(x) {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/cgrkd/updateCgrkd_zibById",
            type: "post",
            data: params,
            dataType: "text",
            // async: false,
    ...
    function delete_zib(x) {
        ...
        if (confirm(msg) == true) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/deleteCgrkd_ZibById",
                type: "GET",
                data: {"cgrkd_zib_id": zibId},
                dataType: "text",
                async: false,
                success: function (data) {
                    alert("删除成功!")
    ...
    function deleteZibByIds(x) {
        ...
        if(confirm(msg) == true){
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/cgrkd/deleteCgrkd_ZibByIds",
                type: "GET",
                data: {"ids": ids},
                dataType: "text",
                async: false,
