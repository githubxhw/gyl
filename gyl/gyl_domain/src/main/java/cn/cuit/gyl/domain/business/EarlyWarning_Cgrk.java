package cn.cuit.gyl.domain.business;

import cn.cuit.gyl.utils.ConverterUtils;

import java.util.Date;

public class EarlyWarning_Cgrk {
    private Long id;//发货预警id
    private String djh;//采购单据单号（主表）
    private Long hh;//行号（子表）
    private String productNum;//商品编码
    private String productName;//商品名称
    private Date yqdhrq;//要求到货日期
    private String yqdhrqStr;
    private Integer checkDays;//预警的天数：在要求发货日期的前checkDays内开始预警
    private Integer invalidDays;//预警失效天数：意思是如果要求发货日期到仍然没有发货，那么依然需要预警（即status状态为1），连续预警invalidDays后就不在预警了（即修改staus为0）；\r\n说明，没有值情况下默认一直预警。
    private Integer status;//预警状态 是否需要预警 1是 0否

    @Override
    public String toString() {
        return "EarlyWarning_Xsck{" +
                "id=" + id +
                ", ddh='" + djh + '\'' +
                ", hh=" + hh +
                ", productNum='" + productNum + '\'' +
                ", productName='" + productName + '\'' +
                ", yqfhrq=" + yqdhrq +
                ", yqfhrqStr=" + yqdhrqStr +
                ", checkDays=" + checkDays +
                ", invalidDays=" + invalidDays +
                ", status=" + status +
                '}';
    }

    public String getYqdhrqStr() {
        return yqdhrqStr;
    }

    public void setYqdhrqStr(String yqdhrqStr) {
        this.yqdhrqStr = yqdhrqStr;
    }

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getDjh() {
        return djh;
    }

    public void setDjh(String djh) {
        this.djh = djh;
    }

    public Long getHh() {
        return hh;
    }

    public void setHh(Long hh) {
        this.hh = hh;
    }

    public String getProductNum() {
        return productNum;
    }

    public void setProductNum(String productNum) {
        this.productNum = productNum;
    }

    public String getProductName() {
        return productName;
    }

    public void setProductName(String productName) {
        this.productName = productName;
    }

    public Date getYqdhrq() {
        return yqdhrq;
    }

    public void setYqdhrq(Date yqdhrq) {
        if(yqdhrq!=null){
            this.yqdhrqStr = ConverterUtils.dateToString(yqdhrq, "yyyy-MM-dd HH:mm");
        }
        this.yqdhrq = yqdhrq;
    }

    public Integer getCheckDays() {
        return checkDays;
    }

    public void setCheckDays(Integer checkDays) {
        this.checkDays = checkDays;
    }

    public Integer getInvalidDays() {
        return invalidDays;
    }

    public void setInvalidDays(Integer invalidDays) {
        this.invalidDays = invalidDays;
    }

    public Integer getStatus() {
        return status;
    }

    public void setStatus(Integer status) {
        this.status = status;
    }
}
